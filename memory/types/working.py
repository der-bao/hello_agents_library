...
class WorkingMemory(BaseMemory):
    ...
    def retrieve(self, query: str, limit: int = 5, user_id: str = None, **kwargs) -> List[MemoryItem]:
        ...
        # 过滤已遗忘的记忆 （不同于过期，遗忘是agent调用forget遗忘的）
        ...
        try:
            ...
            # TF-IDF向量化 （实际应用采用其他嵌入模型，TF - Term Frequency, IDF - Inverse Document Frequency）
    ...
    def _expire_old_memories(self):
        ...
        for m in self.memories:
            if m.timestamp >= cutoff_time:
                ...
                removed_token_sum += len(m.content.split())  # 粗略估算该记忆占用的token数
